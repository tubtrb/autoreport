...
class PublicWebPlaywrightHelperTestCase(unittest.TestCase):
    def test_normalize_version_label_prefixes_plain_version(self) -> None:
        self.assertEqual(normalize_version_label("0.4.2"), "v0.4.2")
        self.assertEqual(normalize_version_label("v0.4.2"), "v0.4.2")

    def test_build_artifact_paths_follow_repo_convention(self) -> None:
        root = Path("C:/repo").resolve()
        paths = build_artifact_paths(root, "0.4.2", "msedge")

        self.assertEqual(
            paths["screenshot_dir"],
            root / "output" / "playwright" / "v0.4.2" / "msedge",
        )
        self.assertEqual(
            paths["download_dir"],
            root / ".playwright-cli" / "downloads" / "v0.4.2" / "msedge",
        )
        self.assertEqual(
            paths["guide_image"],
            root / "docs" / "posts" / "guide-image-v0.4.2" / "image.png",
        )

    def test_promote_guide_image_copies_success_capture(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            success_capture = root / "output" / "playwright" / "v0.4.2" / "msedge" / "generation-success-full.png"
            success_capture.parent.mkdir(parents=True)
            success_capture.write_bytes(b"png-bytes")

            promoted = promote_guide_image(root, "0.4.2", success_capture)
    ...
    def test_build_screenshot_paths_use_action_focused_names(self) -> None:
        screenshot_dir = Path("C:/repo/output/playwright/v0.4.2/msedge")
    ...
    def test_promote_guide_assets_copies_named_screenshots(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            guide_dir = root / "docs" / "posts" / "guide-image-v0.4.2"
            screenshot_dir = root / "output" / "playwright" / "v0.4.2" / "msedge"
            ...
            promoted_dir = promote_guide_assets(root, "0.4.2", screenshot_paths)
    ...
    def test_read_project_version_uses_pyproject_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(read_project_version(root), "0.4.2")
