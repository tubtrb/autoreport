...
- `## Open the hosted demo now` near the top with direct action links for the
  hosted demo and related public pages
- `## What is Autoreport?`
...
- When the guide is meant for `autorelease`, read
  ...
  top action-link section instead of route-only placeholders or a separate
  `## Live service` block.
